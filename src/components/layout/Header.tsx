...
export default function Header({ lang }: { lang: string }) {
  ...
  const { toggleCart, items } = useCart(); // Use Cart Functionality
    ...
    <>
      <header className="sticky top-0 z-50 w-full bg-amina-sand/95 backdrop-blur-sm border-b border-amina-border/40 transition-all duration-300">
        <div className="max-w-7xl mx-auto px-6 h-24 flex items-center justify-between">
          ...
          {/* MOBILE MENU & CART (Left on Mobile) */}
          ...
          <div className="flex items-center gap-4 md:gap-8">
            ...
            <button onClick={toggleCart} className="relative p-2 text-amina-black hover:text-amina-clay transition">
              ...
              {/* Item Count Badge */}
              {items.length > 0 && (
                <span className="absolute top-0 right-0 bg-amina-clay text-white text-[9px] font-bold w-4 h-4 rounded-full flex items-center justify-center">
                  {items.length}
                </span>
              )}
            </button>
      ...
      <div className={`fixed inset-0 z-[60] bg-amina-sand transition-transform duration-500 ease-in-out ${isMenuOpen ? 'translate-y-0' : '-translate-y-full'}`}>
        ...
        <div className="flex flex-col items-center justify-center h-full space-y-8">
          <Link href={`/${lang}`} onClick={closeMenu} className="text-3xl font-serif text-amina-black">Home</Link>
          <Link href={`/${lang}/collection`} onClick={closeMenu} className="text-3xl font-serif text-amina-black">Collection</Link>
          <Link href={`/${lang}/about`} onClick={closeMenu} className="text-3xl font-serif text-amina-black">Our Story</Link>
          ...
          <div className="flex gap-6 text-sm font-bold tracking-widest uppercase text-amina-stone">
             ...
             <Link href={switchLang('fr')}>FR</Link>
             <Link href={switchLang('ar')} className="font-arabic">ع</Link>
          </div>
        </div>
      </div>
    </>
